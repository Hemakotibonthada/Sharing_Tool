...
// ============================================
// LIVE TRANSFER MONITOR WITH CHARTS
// ============================================
...
// Initialize transfer speed chart
function initTransferChart() {
    ...
    transferMonitor.chart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: [],
            datasets: [{
...
// Show transfer monitor
function showTransferMonitor(filename, type = 'upload', fileSize = 0) {
    const monitor = document.getElementById('transferMonitor');
    if (!monitor) return;
    
    transferMonitor.active = true;
    transferMonitor.startTime = Date.now();
    transferMonitor.currentFile = filename;
    transferMonitor.transferType = type;
    transferMonitor.speedHistory = [];
    transferMonitor.peakSpeed = 0;
    transferMonitor.totalTransferred = 0;
    transferMonitor.fileSize = fileSize;
    
    monitor.style.display = 'block';
    
    // Update file info
    document.getElementById('currentTransferFile').innerHTML = `
        <i class="fas fa-file"></i>
        <span>${escapeHtml(filename)}</span>
    `;
    
    const typeBadge = document.getElementById('currentTransferType');
    typeBadge.className = `transfer-type-badge ${type}`;
    typeBadge.innerHTML = type === 'upload' ? 
        '<i class="fas fa-arrow-up"></i><span>Upload</span>' :
        '<i class="fas fa-arrow-down"></i><span>Download</span>';
    
    // Update file size
    document.getElementById('totalFileSize').textContent = formatBytes(fileSize);
    
    // Initialize chart if not exists
    if (!transferMonitor.chart) {
        initTransferChart();
    } else {
        // Reset chart
        transferMonitor.chart.data.labels = [];
        transferMonitor.chart.data.datasets[0].data = [];
        transferMonitor.chart.update();
    }
    
    // Start monitoring interval
    if (transferMonitor.monitorInterval) {
        clearInterval(transferMonitor.monitorInterval);
    }
    transferMonitor.monitorInterval = setInterval(updateTransferMonitor, 500);
}

// Update transfer monitor
function updateTransferMonitor() {
    if (!transferMonitor.active) return;
    
    const elapsed = Math.floor((Date.now() - transferMonitor.startTime) / 1000);
    document.getElementById('transferElapsed').textContent = formatTime(elapsed);
}

// Update transfer progress
function updateTransferProgress(progress, speedMbps) {
    if (!transferMonitor.active) return;
    ...
    // Update speed
    document.getElementById('currentTransferSpeed').textContent = speedMbps.toFixed(2);
    
    // Update speed history
    transferMonitor.speedHistory.push(speedMbps);
    if (transferMonitor.speedHistory.length > transferMonitor.maxDataPoints) {
        transferMonitor.speedHistory.shift();
    }
    
    // Update peak speed
    if (speedMbps > transferMonitor.peakSpeed) {
        transferMonitor.peakSpeed = speedMbps;
        document.getElementById('peakSpeed').textContent = speedMbps.toFixed(2) + ' Mbps';
    }
    
    // Calculate average speed
    const avgSpeed = transferMonitor.speedHistory.reduce((a, b) => a + b, 0) / transferMonitor.speedHistory.length;
    document.getElementById('averageSpeed').textContent = avgSpeed.toFixed(2);
    document.getElementById('avgSpeedDisplay').textContent = avgSpeed.toFixed(2) + ' Mbps';
    
    // Update transferred size
    const transferred = (progress / 100) * transferMonitor.fileSize;
    document.getElementById('transferredSize').textContent = formatBytes(transferred);
    
    // Calculate ETA
    if (speedMbps > 0) {
        const remaining = transferMonitor.fileSize - transferred;
        const speedBytesPerSec = (speedMbps * 1000000) / 8;
        const etaSeconds = remaining / speedBytesPerSec;
        document.getElementById('transferETA').textContent = formatTime(Math.ceil(etaSeconds));
    }
    
    // Update chart
    updateTransferChart(speedMbps);
}

// Update transfer chart
function updateTransferChart(speedMbps) {
    if (!transferMonitor.chart) return;
    ...
    transferMonitor.chart.data.labels.push(timeLabel);
    transferMonitor.chart.data.datasets[0].data.push(speedMbps);
    
    // Keep only last N data points
    if (transferMonitor.chart.data.labels.length > transferMonitor.maxDataPoints) {
        transferMonitor.chart.data.labels.shift();
        transferMonitor.chart.data.datasets[0].data.shift();
    }
    
    transferMonitor.chart.update('none'); // Update without animation for smooth real-time updates
}

// Close transfer monitor
function closeTransferMonitor() {
    const monitor = document.getElementById('transferMonitor');
    if (monitor) {
        monitor.style.display = 'none';
    }
    transferMonitor.active = false;
...
// Show text sharing panel
function showTextSharingPanel() {
    const panel = document.createElement('div');
    panel.className = 'text-sharing-panel';
    panel.id = 'textSharingPanel';
    ...
    panel.innerHTML = `
        <div class="text-panel-header">
            ...
            <button class="btn-icon" onclick="closeTextSharingPanel()" title="Close">
                <i class="fas fa-times"></i>
            </button>
        </div>
        ...
        <div class="text-panel-body">
            ...
            <div class="text-input-section">
                <h4><i class="fas fa-pencil-alt"></i> Share Text</h4>
                <textarea id="textToShare" 
                          placeholder="Type or paste your text here..."
                          rows="5"></textarea>
                <div class="text-input-actions">
                    <button class="btn btn-primary" onclick="shareText()">
            ...
            <!-- Shared Texts List -->
            <div class="shared-texts-section">
                <div class="shared-texts-header">
                    <h4><i class="fas fa-list"></i> Shared Texts</h4>
                    <span class="text-count" id="textCount">0 texts</span>
                </div>
                <div class="shared-texts-list" id="sharedTextsList">
                    <div class="empty-text-state">
                        <i class="fas fa-inbox"></i>
                        <p>No shared texts yet</p>
                        <small>Share some text to get started</small>
                    </div>
                </div>
            </div>
        </div>
    ...
    // Load existing shared texts
    loadSharedTexts();
}

// Close text sharing panel
function closeTextSharingPanel() {
    const panel = document.getElementById('textSharingPanel');
    if (panel) {
...
async function shareText() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast(data.error || 'Failed to share text', 'error');
        }
    } catch (error) {
...
// Render shared texts
function renderSharedTexts() {
    const container = document.getElementById('sharedTextsList');
    ...
    if (sharedTexts.length === 0) {
        ...
        `;
        if (countEl) countEl.textContent = '0 texts';
        return;
    }
    
    if (countEl) countEl.textContent = `${sharedTexts.length} text${sharedTexts.length !== 1 ? 's' : ''}`;
    
    container.innerHTML = sharedTexts.map((item, index) => `
        <div class="shared-text-item">
            <div class="shared-text-header">
                <div class="shared-text-info">
                    <span class="shared-text-author">${escapeHtml(item.author || 'Anonymous')}</span>
                    <span class="shared-text-time">${formatTimestamp(item.timestamp)}</span>
                </div>
                <div class="shared-text-actions">
                    <button class="text-action-btn" onclick="copySharedText(${index})" title="Copy">
                        <i class="fas fa-copy"></i>
                    </button>
                    <button class="text-action-btn" onclick="deleteSharedText('${item.id}')" title="Delete">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            <div class="shared-text-content" id="sharedText${index}">
...
// Copy shared text
function copySharedText(index) {
    const text = sharedTexts[index].text;
